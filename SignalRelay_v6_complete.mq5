//+------------------------------------------------------------------+
//|                                                  SignalRelay.mq5 |
//|                    MT5 -> Telegram via VPS receiver (v6)         |
//| Features: entry+SLTP merge, lock per symbol, ignore adjust       |
//+------------------------------------------------------------------+
#property copyright   "SignalRelay v6 — Hermes Agent"
...
int OnInit()
  {
   ...
   if(InpNotifyStartup && !MQLInfoInteger(MQL_TESTER))
     {
      string msg = "SignalRelay v6 ONLINE\nAccount " + IntegerToString(AccountInfoInteger(ACCOUNT_LOGIN));
...
void OnTradeTransaction(const MqlTradeTransaction& trans, const MqlTradeRequest& request, const MqlTradeResult& result)
  {
   ...
   // Only process OPEN events (not CLOSE/adjust)
   if(!is_open) return;
...
void SendOpenSignal(ulong ticket)
  {
   ...
   // Kirim action sesuai state SL/TP
   string action = (sl <= 0 && tp <= 0) ? "OPEN" : "SLTP";
   
   signalPositionDetails(action, pos_id, ticket);
  }

//+------------------------------------------------------------------+
void signalPositionDetails(string action, ulong pos_id, ulong ticket)
  {
   string symbol = Symbol();
   
   // Cari deal info dari history
   ulong last_deal = HistoryDealsTotal() - 1;
   long dtype = HistoryDealGetInteger(last_deal, DEAL_TYPE);
   string side = (dtype == DEAL_TYPE_BUY) ? "BUY" : "SELL";
   double price = HistoryDealGetDouble(last_deal, DEAL_PRICE);
   double lot = HistoryDealGetDouble(last_deal, DEAL_VOLUME);
   ...
   string json = StringFormat("{\"action\":\"%s\",\"symbol\":\"%s\",\"type\":\"%s\",\"lot\":%.2f,\"price\":%s,\"sl\":%s,\"tp\":%s,\"magic\":%d,\"comment\":\"\",\"position\":%llu,\"digits\":%d}",
      action, symbol, side, lot, DoubleToString(price, digits), DoubleToString(sl, digits), DoubleToString(tp, digits), MagicNumber(), pos_id, digits);
   SendPayload(json);
  }
...
long MagicNumber() { return (InpMagicFilter > 0) ? InpMagicFilter : (long)ParamsGetInteger(1000); }
//+------------------------------------------------------------------+
